 ...
 */
public class ItemExample1_1_Adapter extends HItemAdapter<List<IHItemBean>> {

    public ItemExample1_1_Adapter(int viewType) {
        super(viewType);
    }

    @Override
    public boolean isForViewType(@NonNull List<IHItemBean> items, int position) {
        return items.get(position) instanceof Item1Bean;
    }

    @NonNull
    @Override
    public HViewHolder onCreateViewHolder(ViewGroup parent) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_example1_modular_1, parent, false);
    ...
    public void onBindViewHolder(@NonNull List<IHItemBean> items, int position, @NonNull HViewHolder holder) {
        Item1Bean bean = (Item1Bean) items.get(position);
        holder.setTag(bean);
